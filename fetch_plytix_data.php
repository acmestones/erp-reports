...
// ACTION 1.5: Get status in batches (for progress updates)
if ($action === 'get_status_batch') {
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $pageSize = 25;
    
    error_log("Getting status batch for page $page...");
    ...
    $products = fetchProductList($accessToken, $page, $pageSize);
    ...
    foreach ($products as $product) {
        ...
        $productFile = $cacheDir . '/product_' . $productId . '.json';
        $currentModified = $product['modified'] ?? $product['created'] ?? null;
