...
if ($authHttpCode != 200) {
    die(json_encode(array("error" => "Auth failed", "code" => $authHttpCode)));
}

$authData = json_decode($authResponse, true);

// Fix: Token is in data[0].access_token
if (!isset($authData['data'][0]['access_token'])) {
    die(json_encode(array("error" => "No token in response", "response" => $authData)));
}
...
if ($httpcode == 200) {
    ...
    echo json_encode(array("error" => "Products fetch failed", "http_code" => $httpcode, "response" => json_decode($response, true)));
}
?>
