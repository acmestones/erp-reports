 ...
 * permission settings, attribute configuration, and default filters.
 */
(function () {
    'use strict';
...
function openSettingsModal() {
    ...
    function populateUsersTable() {
        ...
        allSettings.users.forEach(user => {
            ...
            if (user.email === currentUser) {
                tdEmail.innerHTML = `${user.email} <span class="badge bg-info">You</span>`;
            }
    ...
    function populateAttributeCheckboxes(user) {
        ...
        allSettings.available_attributes.forEach(attr => {
            ...
            visibleCheck.innerHTML = `
                <input class="form-check-input visible-attr" type="checkbox" value="${attr}" 
                       id="visible_${attr}" ${isVisibleChecked ? 'checked' : ''}>
            ...
            editableCheck.innerHTML = `
                <input class="form-check-input editable-attr" type="checkbox" value="${attr}" 
                       id="editable_${attr}" ${isEditableChecked ? 'checked' : ''}>
        ...
        // Smart behaviour for individual checkboxes vs All
    ...
    function toggleAllAttributes(type) {
        const checkbox = document.getElementById(type === 'visible' ? 'visibleAll' : 'editableAll');
        const selector = type === 'visible' ? '.visible-attr' : '.editable-attr';
    ...
    function saveUserPermissions() {
        ...
        if (visibleAll) {
            ...
            visibleAttributes = Array.from(
                document.querySelectorAll('.visible-attr:checked')
            ).map(cb => cb.value);
            if (visibleAttributes.length === 0) visibleAttributes = [];
        }
        ...
        if (editableAll) {
            ...
            editableAttributes = Array.from(
                document.querySelectorAll('.editable-attr:checked')
            ).map(cb => cb.value);
            if (editableAttributes.length === 0) editableAttributes = [];
        }
        ...
        fetch('admin_user_settings.php', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // If updating own permissions, refresh userPermissions
        ...
        allSettings.available_attributes.forEach((attr, index) => {
            ...
            row.innerHTML = `
                <div class="d-flex align-items-center flex-grow-1">
                    ...
                    ${isHiddenFromMe ? '<span class="badge bg-secondary ms-2" title="Hidden from your view">Hidden</span>' : ''}
                </div>
                <div>
    ...
    function discoverAllAttributes() {
        ...
        if (newCount === 0) {
            alert('No new attributes found. ' + attributesArray.length + ' attributes are already in the list.');
        } else {
     ...
     */
    function populateDefaultFiltersForm() {
        if (!defaultFiltersConfig) defaultFiltersConfig = getSystemDefaultFilters();

        const searchEl = document.getElementById('defaultSearch');
        const statusEl = document.getElementById('defaultStatus');
        const variantEl = document.getElementById('defaultVariant');
        const sortEl = document.getElementById('defaultSort');

        if (searchEl) searchEl.value = defaultFiltersConfig.search || '';
        if (statusEl) statusEl.value = defaultFiltersConfig.status || 'enabled';
        if (variantEl) variantEl.value = defaultFiltersConfig.variant || 'all';
        if (sortEl) sortEl.value = defaultFiltersConfig.sort || 'sku-asc';

        populateDefaultCategoriesCheckboxes();
        populateDefaultFamiliesCheckboxes();
    }
...
function populateDefaultFamiliesCheckboxes() {
    ...
    function resetDefaultFilters() {
        if (!confirm('Reset to system defaults? This will clear all custom default filter settings.')) {
            return;
        }
    ...
    /**
     * Expose current default filters (for script.js if needed)
     */
    function getDefaultFilters() {
        return defaultFiltersConfig || getSystemDefaultFilters();
    }
    ...
    window.AdminModule = {
        ...
        saveDefaultFilters: saveDefaultFilters,
        resetDefaultFilters: resetDefaultFilters,
        getDefaultFilters: getDefaultFilters,
        populateDefaultCategoriesCheckboxes: populateDefaultCategoriesCheckboxes,
        populateDefaultFamiliesCheckboxes: populateDefaultFamiliesCheckboxes
    };
})();
