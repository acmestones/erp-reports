...
let availableFamilies = {};
  ...
  /**
 * Load default filters configuration
 */
    ...
    fetch(`admin_user_settings.php?action=getDefaultFilters&currentUser=${encodeURIComponent(currentUser)}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                console.error('Failed to load default filters:', data.error);
...
/**
 * Get system default filters (hardcoded fallback)
 */
...
/**
 * Populate the default filters form
 */
function populateDefaultFiltersForm() {
    if (!defaultFiltersConfig) return;

    document.getElementById('defaultSearch').value = defaultFiltersConfig.search || '';
    document.getElementById('defaultStatus').value = defaultFiltersConfig.status || 'enabled';
    document.getElementById('defaultVariant').value = defaultFiltersConfig.variant || 'all';
    document.getElementById('defaultSort').value = defaultFiltersConfig.sort || 'sku-asc';

    // Populate categories checkboxes
    populateDefaultCategoriesCheckboxes();
    
    // Populate families checkboxes
    populateDefaultFamiliesCheckboxes();
}

/**
 * Populate available categories for default filter selection
 */
function populateDefaultCategoriesCheckboxes() {
    const container = document.getElementById('defaultCategoriesCheckboxes');
    if (!container) return;

    // Get categories from main filter or window object
    ...
    availableCategories.forEach(cat => {
        ...
        div.innerHTML = `
            <input class="form-check-input default-category" type="checkbox" 
                   value="${cat}" id="defaultCat_${cat.replace(/[^a-zA-Z0-9]/g, '_')}" 
                   ${isChecked ? 'checked' : ''}>
            <label class="form-check-label" for="defaultCat_${cat.replace(/[^a-zA-Z0-9]/g, '_')}">
                ${cat}
            </label>
        `;
...
function saveDefaultFilters() {
    ...
    const filters = {
        ...
        variant: document.getElementById('defaultVariant').value,
        sort: document.getElementById('defaultSort').value,
    ...
    fetch('admin_user_settings.php', {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Trigger event so script.js can reload defaults
...
/**
 * Reset to system default filters
 */
...
/**
 * Get current default filters (called by script.js)
 */
function getDefaultFilters() {
    return defaultFiltersConfig || getSystemDefaultFilters();
}

// Add to openSettingsModal function (find existing function and add this line)
function openSettingsModal() {
    if (!currentUser) {
        alert('User not initialized');
        return;
